import sys
#python 004.py sample.png

# Start with a file
file = open(sys.argv[1], 'rb')  # Path to ...
data = file.read()
file.close()


# Combine the files
out_file = open('sample2.bytes', 'wb')  # Output file
out_file.write(data)
out_file.close()
...
import sys
with open("sample2.bytes", "rb") as binary_file:
    ...
    for i in range(num_bytes):
        ...
        if eight_bytes == b"\x89\x50\x4e\x47\x0d\x0a\x1a\x0a":  # PNG signature
            ...
            with open("./" + str(i) + "2.png", "wb") as outfile:
